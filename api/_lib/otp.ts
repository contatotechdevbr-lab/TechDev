...
/** Envia o e-mail de redefinição de senha (código de 6 dígitos) via Resend. */
export async function sendPasswordResetEmail(
  ...
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      to: [email],
      ...(replyTo ? { reply_to: replyTo } : {}),
      subject: `${code} é o seu código de redefinição de senha TechDev`,
      html: renderResetEmail(code, saudacao),
      text: `${saudacao}\n\nRecebemos um pedido para redefinir a sua senha na TechDev.\nSeu código de redefinição é: ${code}\n\nEle expira em ${OTP_TTL_MINUTES} minutos. Se você não solicitou, ignore este e-mail — sua senha continua a mesma.`,
    }),
  });
...
/** Template HTML do e-mail de redefinição de senha (inline styles). */
function renderResetEmail(code: string, saudacao: string): string {
  return `<!doctype html>
<html lang="pt-BR">
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background-color:#121821;border:1px solid #1f2937;border-radius:16px;overflow:hidden;">
            ...
            <tr>
              <td style="padding:8px 32px 0 32px;">
                <h1 style="margin:0 0 8px 0;font-size:20px;line-height:1.3;color:#f1f5f9;">Redefinição de senha</h1>
                <p style="margin:0;font-size:14px;line-height:1.6;color:#94a3b8;">${saudacao} Recebemos um pedido para redefinir a sua senha. Use o código abaixo para criar uma nova. Ele expira em ${OTP_TTL_MINUTES} minutos.</p>
              </td>
            </tr>
            <tr>
            ...
            <tr>
              <td style="padding:0 32px 32px 32px;">
                <p style="margin:0;font-size:12px;line-height:1.6;color:#64748b;">Se você não solicitou a redefinição, ignore este e-mail com segurança — sua senha atual continua válida.</p>
              </td>
            </tr>
          </table>
